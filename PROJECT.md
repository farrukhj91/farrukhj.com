...
**Last updated:** 2026-06-22 (Phase 4 Round 6)
...
| 2026-06-15 R5 | About narrative rewritten in Farrukh's voice from his raw answers to section 17B prompts. Replaces Claude-voiced career walkthrough that overlapped Experience. Six paragraphs + retained learnings list + new outside-of-work paragraph before contact close. Lead Pursuits intentionally not named (Experience carries it) | Voice samples: Farrukh's Medium/LinkedIn posts (financial literacy, NPS critique, UX fellowship grad). Calibrated tone: longer sentences, plain connectives, no em-dashes, direct emotional statements, source-name visibility (Fahim Akhter, Attabad lake) |
| 2026-06-15 R5 | Monogram pick (section 17A) deferred — Farrukh designing externally and will provide SVG | Three text-stroke variants (A/B/C) rejected; faceted geometric direction (v1 with 14 triangles, v2 with solid letterforms + 4 seams) also rejected. Farrukh handling offline |
...
| 2026-06-22 R6 | Reference compared: awaisimran.com (WordPress/Semplice designer portfolio). Logged in plan file. NOT added to reference triangle — triangle stays Linear/Vercel/Chimero. One-page architecture explicitly rejected as incompatible with long-form case-study + TOC posture | Plan file: `~/.claude/plans/abstract-prancing-stream.md` |
| 2026-06-22 R6 | Move 3 shipped: `collaborators` entries annotated with role credentials (Tooba Malik → "Tooba Malik (Product Designer)") across offline-lms.md and stakeholder-dashboards.md. RoleFooter unchanged | Product already disambiguated by case-study mono meta strip — only role added |
| 2026-06-22 R6 | Move 4 shipped: `.num` mono treatment applied to 8 in-prose numeric anchors across 3 case studies — outcome blockquote in LMS Redesign (`15–20` / `5` / `2` clicks), `35` TCF schools + `70%` reduction in Offline LMS, `94%` / `80%` / `93.5%` / `71.7%` / `73.9%` in Stakeholder Dashboards. `.num` was already global in `base.css:62` — no CSS lift needed | Conservative pass — only load-bearing metrics, not every digit |
| 2026-06-22 R6 | Header portrait swap evaluated: two new candidate sources processed (waistcoat-at-door, rooftop-evening). Neither beat current `farrukh-header.jpg` on daylight + place signal + over-shoulder engagement. Current shot kept; candidates discarded | New sources stay in `_inbox/Pictures of Me/` |
| 2026-06-22 R6 | `§` symbol audit: user reported seeing `§` "in many places and in case studies." Grep across `src/`, `public/`, `*.md`, `*.astro`, `*.css` found ZERO `§` characters in any rendered surface or content file. Only 3 occurrences existed — all in PROJECT.md internal cross-references (`§17A`, `§17B`, `§17`) — and have been replaced with "section " prose. If `§` is appearing on the live site, source is elsewhere — needs user to point at a specific page | |
| 2026-06-22 R6 | All Tooba Malik mentions hyperlinked to LinkedIn (`https://www.linkedin.com/in/toobamalick/`). 8 mentions covered: 6 body-prose mentions across both case studies use markdown `[Tooba](url)`; 2 frontmatter `summary.approach` strings use `<a>` tag (required teaching `ExecutiveSummary` to render lines via `set:html`); 1 italicized role-footer line in each case study uses markdown link. Alt text not linked — assistive tech reads alt as plain text | `ExecutiveSummary.astro` lines 10–12 changed from `{summary.x}` to `set:html={summary.x}`. Internal content is trusted — `set:html` XSS surface is the YAML authoring step |
| 2026-06-22 R6 | ChatGPT portfolio review applied selectively. **Shipped**: hero sentence rewrite (added "Senior" prefix + scope/markets clause — "Eight years across EdTech, SaaS, and enterprise platforms"); footer Résumé link added (`/resume.pdf` already lived, was just unlinked from footer). **Rejected**: dedicated AI section (violates §2 "no AI in nav/headings"); homepage section reorder to one-page (violates docs-site posture); bulletized role block (italic role-footer line already does this senior-typographically); Resume/LinkedIn/Email block in hero (hero stays quiet — footer absorbs). **Partially noted**: light editorial pass to surface PM verbs in case studies (user-owned content edit, not yet scheduled) | Plan file: `~/.claude/plans/abstract-prancing-stream.md`. Review treated case-study ExecutiveSummary as nonexistent (it's literally the Challenge/Approach/Outcome framework it asked for) — surfacing this back to the user |
